...
class DiseaseManager extends Model {
    ...
    public function getDiseases() {
        ...
        if ($page == "home") {
            $result = $this->getBdd()->prepare_execute($sql .= " Limit 10", $params);
        } else {
            $result = $this->getBdd()->prepare_execute($sql .= " Limit 500", $params);
        }
        ...
        if(isset($_SESSION['id'])){
            ...
            $fav = $this->prepare_execute($sql, $params);
        ...
        // return json_encode($resultsArray, JSON_UNESCAPED_UNICODE);
        return $resultsArray;
    }

    public function getMeridiens() {
        $this->getBdd();
        return $this->getAll('meridien', 'Meridien'); //construit un objet Meridien.php par ligne de la table meridien
    }

    public function isPathoTableNotEmpty() {
        $this->getBdd();
        return ($this->notEmpty("patho"));
    }
}